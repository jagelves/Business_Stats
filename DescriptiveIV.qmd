...
Unlike variance, MAD considers absolute deviations from the mean, providing a measure that is less sensitive to outliers. The **Mean Absolute Deviation** ($MAD$) measures the average deviation from the mean. This measure uses absolute deviations. It is calculated by $MAD=\frac{\sum |x_i-\mu|}{N}$ for the population and $mad=\frac{\sum |x_i-\bar{x}|}{n}$ for the sample.
