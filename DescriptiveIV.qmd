...
Ideally, our measure of dispersion would simply average the distances from each data point to the mean, giving us a clear indication of how much the data varies around this central point. However, because the mean is calculated to balance out these distances, the sum of all deviations from the mean equals zero. This balancing effect is visually apparent in the graph, where positive deviations are offset by negative ones.

To address this issue, variance squares each deviation from the mean before averaging them. This squaring eliminates negative values, ensuring a non-zero measure of spread. The table below illustrates these calculations:

```{r echo=FALSE, out.width="75%", fig.align='center'}
knitr::include_graphics("images/Var1.png")
```
The table shows the deviations from the mean in the second column. You can convince yourself that these add up to zero. The third column squares the deviation. The variance, averages the numbers in the third column. The result is $s^2=977594$, which means that the price of Ether varies on average 977,594 squared deviations from the mean. The result by itself is not very intuitive, however if we compare with another variable, we can assess whether Ether is more or less variable.

## The Standard Deviation

The standard deviation is derived by taking the square root of the variance. Remember, variance employs squared deviations to eliminate negative values and calculate spread. By taking the square root of this variance, the standard deviation reverts the measure of dispersion back to the original units of the variable. This transformation makes the standard deviation more intuitive; it directly quantifies how much each data point deviates from the mean in the same units as the variable itself. Consequently, the standard deviation is a clear and intuitive measure of variability.
...
Another measure of data variability is the Mean Absolute Deviation (MAD), which calculates variability using the absolute deviations from the mean, rather than squaring them like variance does. This approach not only keeps the measure in the same units as the data but also makes it less sensitive to outliers. Practically, the **Mean Absolute Deviation** measures the average deviation from the mean, by using absolute deviations. It is calculated by $MAD=\frac{\sum |x_i-\mu|}{N}$ for the population and $mad=\frac{\sum |x_i-\bar{x}|}{n}$ for the sample.
...
For the MAD, focus on the fourth column which lists the absolute deviations from the mean. Averaging these gives us the Mean Absolute Deviation (MAD), which equals $MAD=899.2$. This means, on average, each data point is roughly 900 dollars away from the mean. Note that the standard deviation is higher ($988.73$) because squaring deviations disproportionately amplifies larger ones, pushing the standard deviation above the MAD.
