...
The two formulas look very similar, but there is one key difference: the population variance is divided by $N$, while sample variance is divided by $n − 1$. This small change is known as **Bessel’s correction**. The reason is simple. When we calculate the sample variance, we replace the true (but unknown) population mean $\mu$ with the sample mean $\bar{x}$. Because $\bar{x}$̄ is deliberately chosen to minimize the sum of squared deviations from the data points themselves, that sum ends up being artificially smaller than it would be if we had used the true $\mu$. If we divided by $n$, the result would systematically underestimate the true population variance $\sigma^2$ on average. Dividing by $n − 1$ instead provides the right adjustment to correct this bias.
...
The monthly returns (in percent) for a small project are given in the table below:
...
Compute the number of observations, sample mean, sample variance, sample standard deviation, coefficient of variation (CV), and the mean absolute deviation (MAD). Capture the data in a tibble and find the statistics using the `summarise()` function. Briefly interpret the CV: what does it tell you about the returns relative to their mean?
