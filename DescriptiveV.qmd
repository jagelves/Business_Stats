...
There are statistical measures that describe the shape and distribution of data beyond simple measures of central location or dispersion. They provide a nuanced view of how data is spread out, where it concentrates, and how it deviates from what might be expected. Quantiles and percentiles give specific points of division within the data, highlighting where certain proportions lie. Chebyshev's Theorem and the Empirical Rule explain how much data falls within certain ranges from the mean, providing a framework for normalcy or expected variability across different distributions. Skewness adds insight into the symmetry of the distribution, revealing biases in data distribution. The five-point summary and resulting interquartile range (IQR) offer a robust measure of spread, focusing on the middle 50% of the data to avoid the influence of outliers. Finally, box plots visualize this summary alongside outliers, giving a complete picture of data distribution. Together, these tools quantify central tendency and variability and illustrate the data's shape, symmetry, and anomalies, offering a fuller understanding of the dataset's characteristics. Below we take a look at these measures in detail.

## Quantiles and Percentiles

Quantiles and percentiles are values beneath which a certain (proportion) percentage of the data in a data set is found. Specifically, if we are using quantiles we express ratios, for percentiles we use percentages.
...
Formally, a **quantile** is a location within a set of ranked numbers (or distribution), below which a certain proportion, $q$, of that set lie. Ex: 0.45 of the data lies below the 0.45 quantile. **Percentiles** express quantiles in percentage form. Ex: 45% of the data lies below the 45th percentile.  
