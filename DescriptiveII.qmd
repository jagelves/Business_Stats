...
Alternatively, you can use Sturges' rule to estimate the number of classes. This rule suggests using
$ k = \lceil 1 + 3.322 \log_{10}(n) \rceil $
where $n$ is the number of observations and $\lceil \cdot \rceil$ denotes the ceiling function (round up to the nearest integer).

**Ex2:** If a data set has 50 observations,
$ k = \lceil 1 + 3.322 \times 1.699 \approx 6.64 \rceil = 7 $ so we would choose seven classes.
