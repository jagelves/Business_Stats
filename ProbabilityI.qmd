...
The **Permutation** function also counts the number of ways to choose $x$ objects from a total of $n$ objects. However, the order in which the $x$ objects are listed does matter. If repetition is not allowed we use:

$$P(n,x)= \frac{n!}{(n-x)!}$$

Ex: The Olympic medal finals for the 100 meters sprint has eight runners competing for three medals. How many medal podiums are possible? There are a total of . Below you can see the calculation.
