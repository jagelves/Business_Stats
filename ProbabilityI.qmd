...
$$C(n, x)= \frac{n!}{(n-x)!x!}$$
Ex: Consider the starting three batters for a baseball team. In how many 
...
The **Permutation** function also counts the number of ways to choose $x$ objects from a total of $n$ objects. However, the order in which the $x$ objects are listed does matter. If repetition is not allowed use 
