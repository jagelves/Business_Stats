...
The **slope** determines the steepness of the line. The estimate quantifies how much a unit increase in $x$ changes $y$. We can easily calculate the slope by using the covariance between $y$ and $x$, and the variance of $x$. Mathematically, $\hat{\beta}= \frac {s_{xy}}{s_{x}^2}$.
