...
Quantifying relationships between variables is an important skill in business. The regression line represents the best linear fit between two variables. This technique, allows businesses to make predictions, identify trends, and optimize strategies based on data-driven evidence. Below, we study how to generate and analyze the regression line output.
...
**Example: Let's consider some data on Price and Advertisement. Below you can see the data:

```{r echo=FALSE, fig.align='center'}
knitr::include_graphics("images/reg1.png")
```
The data shows how the two variables are related. Since, the data set is small, we can observe that when advertisement is high price tends to be also high. There is a direct relationship. In our previous chapter we learned how to quantify this relationship with the covariance and the correlation coefficient. 

In this section, we want to quantify to things. First, if we have a budget of 6 for advertisement, what is the price? Second, how much are we able to increase price for every extra dollar spent in advertisement? The regression line allows us to answer these questions. 

Let's start by calculating the regression line. To do this we will need the variance of advertisement, the covariance between the two variables, and the means for both variables. The slope of the regression line is given by $\hat{\beta}= \frac {s_{xy}}{s_{x}^2}$. Given that the covariance is 3.67m and the variance of x is 1.67, the slope of the regression line is $\hat{\beta}= \frac{3.67}{1.67}=2.2$. This means that for every dollar increase in advertisement, we can increase the price by 2.2.

Next, let's calculate the intercept so that we can complete the regression line. The intercept is given by $\hat{\alpha}=\bar{y}-\hat{\beta}\bar{x}$. Since the mean of advertisement is 2.5 and the mean of price is 7, the intercept is $\hat{\alpha}=7-2.2(2.5)=1.5$. This means that if we do not advertise, the predicted price is zero. 

The regression line can be completed by using the intercept and slope that we have estimated above. In particular, the regression line is $\hat{y_i}=1.5+2.2x_i$. With this equation we can now establish that if we had a budget of 6 for advertisement, our predicted price would be $\hat{y_i}=1.5+2.2(6)=14.7$.

In conclusion, the regression line has helped us predict what prices would be given a budget, and helped us quantify how effective is advertisement at increases the price.
