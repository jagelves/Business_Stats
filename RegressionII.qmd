...
The table shows that x "closes the gap" with respect to the mean prediction by $17.63$. If we add calories to our model, we further decrease the gap by another $6.55$. In sum, using them together increases the $SSR$ to $24.18$, which in turn reduced the $SSE$ to only $1.01$. Finally, the $R^2$ also increases from $0.7$ to now $0.96$, $R^2= \frac{17.63+6.55}{25.2}$.
